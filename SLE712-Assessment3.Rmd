...
res
```
using the mutator to mutate the original sequence and then using the blast command to import and compare the changes in the mismatches of which from the original sequence the mismatch was 0 and when the original sequence was mutated the mismatch turns to 19 and there is a noticeable change in the bitscore and p.identity.
